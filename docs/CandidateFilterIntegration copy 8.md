...
## Response: `matchedCertifications` (Professional Certifications match summary)
...
| Field | Type | Notes |
|-------|------|-------|
| `candidateCertificationId` | `number` | Stable id of `candidate_certifications`; one entry per matching row. |
| `certificationId` | `number \| null` | Catalog certification id on the row. |
| `certificationName` | `string \| null` | Certification display name (Cards heading). |
| `matchedByCertificationId` | `boolean` | `true` only when `certificationId` filter is active and this row's catalog id matched. |
| `issuingBody` | `{ id: number, label: string } \| null` | Intersection with requested `issuingBodyIds` filter. |
| `level` | `{ id: number, label: string } \| null` | Intersection with requested `certificationLevels` filter. |
| `issueDate` | `string \| null` (ISO `DateOnly`) | Row issue date for contextual display; not filter-driven. |
| `expiryDate` | `string \| null` (ISO `DateOnly`) | Row expiry date for contextual display; not filter-driven. |
...
- **Row inclusion:** when both `certificationId` and `issuingBodyIds` are active, a row qualifies if its catalog id matches **or** its issuer matches; when `certificationLevels` is also active, the row must additionally match level. See `docs/matched_certifications_backend_prompt.md` §6 for full rules.
- **Matched-only:** fields hold only intersection with active filters; inactive sub-filters → `null` / `matchedByCertificationId: false`.
- **One entry per candidate certification row** — not deduped by catalog `certificationId`.
- Items ordered by `candidateCertificationId` ascending.
