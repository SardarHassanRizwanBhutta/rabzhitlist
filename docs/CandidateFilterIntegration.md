...
**AchievementType** enum (same integers as create/update): `0` Competition, `1` Open Source, `2` Award, `3` Medal, `4` Publication, `5` Certification, `6` Recognition, `7` Other.
...
## Response: `matchedAchievements` (Achievement match summary)

Each `CandidateListItemDto` includes a **`matchedAchievements`** array so the Cards View can render, per candidate,
which achievement rows caused the match (achievement name heading + matched-value badges).
...
| `name` | `string` | Achievement display name (Cards heading). |
...
- **Matched-only:** a row appears only if it matched ≥1 **active** driver filter (OR across type and name filters).
...
- Items ordered by `achievementId` ascending. Not deduped by name.
