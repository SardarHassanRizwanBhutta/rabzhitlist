...
Work experience driver filters trigger **`matchedWorkExperiences`** on the list response (see below). They do **not** trigger `matchedEmployers` or `matchedProjects`.
...
Otherwise → `matchedEmployers: []` (never `null`). Work-experience row filters (`shiftTypes`, `workModes`, `timeSupportZoneIds`, `workExperienceTechStackIds`) do not trigger population; use `matchedWorkExperiences` instead. Candidate-level filters alone (`jobTitle`, `minExperienceYears`, etc.) do not trigger either payload.
...
## Response: `matchedWorkExperiences` (Work Experience match summary)

Each `CandidateListItemDto` includes a **`matchedWorkExperiences`** array so the Cards View can render, per candidate,
which work experiences caused the match (`{employerName} - {jobTitle}` heading + matched-value badges).
...
| `workExperienceId` | `number` | Stable id of `CandidateWorkExperience`; one entry per matching work experience. |
| `employerId` | `number` | Linked employer id (non-deleted). |
| `employerName` | `string` | Employer display name (part of UI heading). |
| `jobTitle` | `string \| null` | From the work experience row. |
| `startDate` | `string \| null` (ISO `DateOnly`) | Work experience start date. |
| `endDate` | `string \| null` (ISO `DateOnly`) | Work experience end date; `null` = current role. |
| `shiftType` | `{ id: number, label: string } \| null` | When `shiftTypes` filter active and this WE's shift matched. Scalar (one enum per row). |
| `workMode` | `{ id: number, label: string } \| null` | When `workModes` filter active and this WE's work mode matched. Scalar (one enum per row). |
| `timeSupportZones` | `{ id: number, label: string }[]` | Intersection with requested `timeSupportZoneIds` filter. |
| `techStacks` | `{ id: number, label: string }[]` | Intersection with requested `workExperienceTechStackIds` filter (WE stacks only, not project `techStackIds`). |
...
- Each field holds only the **intersection** with its filter. When a sub-filter is not applied: arrays → `[]`; scalars (`shiftType`, `workMode`) → `null`.
- Enum `{ id, label }` objects use the **same integer** as the query-param filter; `label` is server-normalized display text.
- Values are **distinct by `id`** within arrays, ordered **ascending by `id`**; items ordered by `workExperienceId`.
- The same `workExperienceId` may also appear in `matchedEmployers` when employer driver filters are active; payloads are independent.

Full backend brief: `docs/matched_work_experiences_backend_prompt.md`.
