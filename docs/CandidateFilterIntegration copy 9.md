...
**CertificationLevel** enum (same integers as create/update): `0` Foundation, `1` Associate, `2` Professional, `3` Expert, `4` Master.

Certification driver filters trigger **`matchedCertifications`** on the list response (see below).
...
Education driver filters trigger **`matchedEducations`** on the list response (see below).
...
Achievement driver filters trigger **`matchedAchievements`** on the list response (see below).
...
## Response: `matchedEducations` (Education Background match summary)

Each `CandidateListItemDto` includes a **`matchedEducations`** array so the Cards View can render, per candidate,
which education rows caused the match (university name heading + matched-value badges).
...
| `grades` | `string \| null` | Row grades for display when non-empty (not filter-driven). |
| `isTopper` | `boolean \| null` | `true` only when `isTopper=true` filter active and row is topper; otherwise `null`. |
| `isMainCheetah` | `boolean \| null` | `true` only when `isMainCheetah=true` filter active and row is main cheetah; otherwise `null`. |
...
- **Matched-only:** an education row appears only if it matched ≥1 **active** driver filter (OR across drivers).
- **One entry per education row** — not deduped by university.
- Each field holds only the **intersection** with its filter; inactive sub-filters → `null` / `matchedByUniversityId: false` as documented.
- Items ordered by `educationId` ascending.

---

## Response: `matchedCertifications` (Professional Certification match summary)

Each `CandidateListItemDto` includes a **`matchedCertifications`** array so the Cards View can render, per candidate,
which certification rows caused the match (certification name heading + matched-value badges).

```jsonc
"matchedCertifications": [
  {
    "candidateCertificationId": 55,
    "certificationId": 55,
...
| Field | Type | Notes |
|-------|------|-------|
| `candidateCertificationId` | `number` | Row identifier. Current schema uses composite PK `(candidate_id, certification_id)` with no surrogate `id`; equals catalog `certificationId` (unique per candidate). |
| `certificationId` | `number` | Catalog certification id on the row. |
| `certificationName` | `string \| null` | Catalog display name (Cards heading). |
| `matchedByCertificationId` | `boolean` | `true` only when `certificationId` filter active and this row's catalog id matched. |
| `issuingBody` | `{ id: number, label: string } \| null` | Intersection with requested `issuingBodyIds` filter. |
| `level` | `{ id: number, label: string } \| null` | Intersection with requested `certificationLevels` filter. Enum `id` = query param integer. |
| `issueDate` | `string \| null` (ISO `DateOnly`) | Row issue date when present; contextual display. |
| `expiryDate` | `string \| null` (ISO `DateOnly`) | Row expiry date when present; contextual display. |

**When `matchedCertifications` is computed:** any of these filters is active:

- `certificationId`, `issuingBodyIds`, `certificationLevels`

Otherwise → `matchedCertifications: []` (never `null`).

**Row inclusion** (per certification row, mirrors frontend Cards mock):

- `certIssuerOk`: when both cert id and issuer filters active, row matches if catalog id **or** issuer matches; otherwise match the single active filter; when neither active, all rows pass this gate (level-only filter).
- Row included when `certIssuerOk && (!levelFilterActive || levelMatches)`.

**Matched-only fields:** each field holds only the intersection with its filter; inactive sub-filters → `null` / `matchedByCertificationId: false`.

Items ordered by `candidateCertificationId` ascending. Not deduped by catalog id (future schema may allow duplicate catalog rows per candidate).

---

## Response: `matchedAchievements` (Achievements match summary)

Each `CandidateListItemDto` includes a **`matchedAchievements`** array so the Cards View can render, per candidate,
which achievement rows caused the match (achievement name heading + matched-value badges + ranking/year context).
...
| `matchedByAchievementName` | `boolean` | `true` only when `achievementName` filter active and this row's name matched via substring. |
| `achievementType` | `{ id: number, label: string } \| null` | Intersection with requested `achievementTypes` filter. |
...
**When `matchedAchievements` is computed:** any of these filters is active:

- `achievementTypes`, `achievementName`

Otherwise → `matchedAchievements: []` (never `null`).

Semantics:

- **Matched-only:** a row appears only if it matched ≥1 **active** driver filter (OR across type and name).
- **One entry per achievement row** — not deduped by name.
- Each field holds only the **intersection** with its filter; inactive sub-filters → `null` / `matchedByAchievementName: false`.
